...
const signupUrl = 'https://script.google.com/macros/s/AKfycbxJOD0hzk901LqjtdVn1atb9Twts0FbMtTrWbcy5ddpB-lw9aXj/exec';
...
signupForm.addEventListener('submit', function(e) {
  ...
  xmlhttp.open('GET', `${signupUrl}?email=${email}&phone=${phone}&fullname=${fullname}&promocode=${promocode}`, true);
  xmlhttp.send();
...
const discountUrl = 'https://script.google.com/macros/s/AKfycbz7zW0LIllJ134ePgVC0BuzH07blMoirFsPljOT3j7qLsxhaxo/exec';
...
discountForm.addEventListener('submit', function(e) {
  ...
  xmlhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
        let response = JSON.parse(this.responseText);
        discountPopup.querySelector('.popup__success').classList.remove('hidden');
        setTimeout(function() {
          discountPopup.classList.add('visually-hidden');
          body.classList.remove('body__overlay');
          discountPopup.querySelector('.popup__success').classList.add('hidden');
        }, 3000);
    }
  };
})
